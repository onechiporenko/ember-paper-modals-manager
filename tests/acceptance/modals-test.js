...
import declineModal from '../helpers/decline-modal';
import modalIsOpened from '../helpers/modal-is-opened';
import confirmButtonState from '../helpers/confirm-button-state';
import promptValue from '../helpers/prompt-value';
import customModalText from '../helpers/custom-modal-text';
import confirmModal from '../helpers/confirm-modal';
import lastLogMessageAssert from '../helpers/last-log-message-assert';
...
module('Acceptance | modals', function (hooks) {
  ...
  test('alert-modal', async function (assert) {
    ...
    await customModalText(assert, 'header', 'Custom Alert Modal Title');
    await customModalText(assert, 'body', 'Custom Alert Modal Body');
    await confirmModal();
  ...
  test('Custom alert-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    await customModalText(assert, 'header', 'Custom Alert Title Component');
    await customModalText(assert, 'body', 'Custom Alert Body Component');
    await customModalText(assert, 'footer', 'Custom Alert Footer Component');
    await confirmModal();
    await modalIsOpened(assert, false);
  ...
  test('confirm-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    await customModalText(assert, 'header', 'Custom Confirm Modal Title');
    await customModalText(assert, 'body', 'Custom Confirm Modal Body');
    await confirmModal();
    await modalIsOpened(assert, false);
  ...
  test('Custom confirm-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    await customModalText(assert, 'header', 'Custom Confirm Title Component');
    await customModalText(assert, 'body', 'Custom Confirm Body Component');
    await customModalText(assert, 'footer', 'Custom Confirm Footer Component');
    await confirmModal();
    await modalIsOpened(assert, false);
  ...
  test('prompt-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    await customModalText(assert, 'header', 'Custom Prompt Modal Title');
    await customModalText(assert, 'body', 'Custom Prompt Modal Body');
    await customModalText(assert, 'body', 'Input Label');
  ...
  test('Custom prompt-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    await customModalText(assert, 'header', 'Custom Prompt Title Component');
    await customModalText(assert, 'body', 'Custom Prompt Body Component');
    await customModalText(assert, 'footer', 'Custom Prompt Footer Component');
    await promptValue('test');
    await confirmModal();
    await modalIsOpened(assert, false);
  ...
  test('prompt-confirm-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    confirmButtonState(assert, true);
    await customModalText(assert, 'header', 'Custom Prompt Confirm Modal Title');
    await customModalText(assert, 'body', 'Please enter a "modal" without quotes');
    await customModalText(assert, 'body', 'Input Label');
  ...
  test('Custom prompt-confirm-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    confirmButtonState(assert, true);
    await customModalText(assert, 'header', 'Custom Prompt Confirm Title Component');
    await customModalText(assert, 'body', 'Custom Prompt Confirm Body Component');
    await customModalText(assert, 'body', 'Please enter a "modal" without quotes');
    await customModalText(assert, 'footer', 'Custom Prompt Confirm Footer Component');
    await promptValue('modal');
    confirmButtonState(assert, false);
    await confirmModal();
    await modalIsOpened(assert, false);
  ...
  test('check-confirm-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    confirmButtonState(assert, true);
    await customModalText(assert, 'header', 'Custom Check Confirm Modal Title');
    await customModalText(assert, 'body', 'Confirm your suggestion');
    await customModalText(assert, 'body', 'Input Label');
  ...
  test('Custom check-confirm-modal', async function (assert) {
    ...
    await modalIsOpened(assert, true);
    confirmButtonState(assert, true);
    await customModalText(assert, 'header', 'Custom Check Confirm Title Component');
    await customModalText(assert, 'body', 'Custom Check Confirm Body Component');
    await customModalText(assert, 'footer', 'Custom Check Confirm Footer Component');
    await click('md-dialog-content md-checkbox');
    confirmButtonState(assert, false);
    await confirmModal();
    await modalIsOpened(assert, false);
  ...
  test('progress-modal (error)', async function (assert) {
    ...
    setTimeout(async () => {
      ...
      await customModalText(assert, 'body', 'Promise was rejected');
  ...
  test('Custom progress-modal (success)', async function (assert) {
    ...
    const done = assert.async();
    await modalIsOpened(assert, true);
    await customModalText(assert, 'header', 'Custom Progress Title Component');
    await customModalText(assert, 'body', 'Custom Progress Body Component');
    await customModalText(assert, 'body', '/ 5');
    await customModalText(assert, 'footer', 'Custom Progress Footer Component');
    assert.ok(find('md-dialog-content md-progress-linear'), 'Progress bar exists');
    assert.notOk(find('md-dialog-actions button'), 'Footer is without buttons');
    setTimeout(async () => {
      await modalIsOpened(assert, false);
